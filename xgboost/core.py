...
''' BEGIN CHANGES '''
from .pareto import find_pareto_descent  # To use in Booster.update()
''' END CHANGES '''
...
''' BEGIN CHANGES '''
def create_DMatrices(X_train, y_train_ls):
    """
    Returns a list where each element is a DMatrix corresponding to the features
    and one set of labels. Every element has the same features and different
    labels, each corresponding to a single objective.
    """
    dtrain_ls = []
    for y_train in y_train_ls:
        dtrain_ls.append(DMatrix(X_train, y_train))
    return dtrain_ls
...
#
# _X_full = np.load(path + 'X_full.npy')
# _y_full = np.load(path + 'y_full.npy')
#
# _X_train, _X_test, _y_train, _y_test = train_test_split(_X_full, _y_full,
#                                                         test_size=0.20,
#                                                         random_state=1)
#
# _dtrain_ls = create_DMatrices(_X_train,
#                               [_y_train, _y_train, _y_train, _y_train])
...
# xgmat = DMatrix( data, label=label, missing = -999.0, weight=weight )
#
...
class Booster(object):
    ...
    def update(self, dtrain, iteration, fobj=None):
        ...
        if fobj is None:
            ...
            # Initialize grad_all and hess_all arrays to fill inside for-loop,
            # with one row for every point and one column for every objective
            n_points = _dtrain_ls[0].num_row()
            n_objs = len(_dtrain_ls)
            grad_all = np.full((n_points, n_objs), np.nan)
            hess_all = np.full((n_points, n_objs), np.nan)

            # Calculate gradient and hessian (first and second derivative) for
            # every objective
            for i, dtrain_i in enumerate(_dtrain_ls):

                pred = self.predict(dtrain_i)
                grad, hess = fobj(pred, dtrain_i)

                grad_all[:, i] = grad.ravel()
                hess_all[:, i] = hess.ravel()

            # Find best descent direction for all objectives
            grad_pareto = find_pareto_descent(grad_all.astype(np.double)).ravel()
            hess_pareto = find_pareto_descent(hess_all.astype(np.double)).ravel()
